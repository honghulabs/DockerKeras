...
# # Install Keras for MXNET (experimental).
# RUN pip3 --no-cache-dir install keras-mxnet
RUN git clone https://github.com/deep-learning-tools/keras /opt/keras && \
    cd /opt/keras && \
    git fetch && \
...
RUN mkdir /root/.keras && \
    wget -O /root/.keras/keras.json https://raw.githubusercontent.com/chi-hung/DockerbuildsKeras/master/keras-mxnet.json
